...
func TestMemoryAbsoluteInt(t *testing.T) {
	...
	m.WriteMemoryAddressModes(1, addressing.Absolute(3))
	assert.Equal(t, 1, m.ReadMemory(2))
	assert.Equal(t, 1, m.ReadMemoryAddressModes(false, addressing.Absolute(3)))
}
...
func TestMemoryAbsoluteIndirect(t *testing.T) {
	...
	*x = 1
	m.WriteMemoryAddressModes(1, addressing.Indirect(2), x)
	assert.Equal(t, 1, m.ReadMemory(0x10))

	m.WriteMemory(8, 0x00)
	m.WriteMemory(9, 0x18)
	*y = 1
	m.WriteMemoryAddressModes(1, addressing.Indirect(8), y)
